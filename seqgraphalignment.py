...
class SeqGraphAlignment(object):
    ...
    def __init__(self, sequence, graph, score=[4,-3,-3,0], fastMethod=True, globalAlign=False, *args, **kwargs):
        self.sequence   = sequence
        self.graph      = graph
        self.stringidxs = None
        self.nodeidxs   = None
        self.globalAlign= globalAlign
        self.matchscore= score[0]
        self.mismatchscore= score[1]
        self.opengapscore= score[2]
        self.extendgapscore= score[3]
        if fastMethod:
    ...
    def alignStringToGraphSimple(self):
        ...
        insertCost = collections.defaultdict(lambda : self.opengapscore)
        deleteCost = collections.defaultdict(lambda : self.opengapscore)
        ...
        for i,node in enumerate(ni()):
            ...
            for j,sbase in enumerate(self.sequence):
                ...
                if movetype == "INS":
                    insertCost[(i+1,j+1)] = self.extendgapscore
                if movetype == "DEL":
                    deleteCost[(i+1,j+1)] = self.extendgapscore
    ...
    def alignStringToGraphFast(self):
        ...
        # this location
        insertCost = numpy.zeros((l1+1,l2+1),dtype=numpy.int)+self.opengapscore
        deleteCost = numpy.zeros((l1+1,l2+1),dtype=numpy.int)+self.opengapscore
        ...
        def insertions(inserted, insscores, l2, scores, insertCost, backStrIdx):
            ...
            for j in range(l2):
                if insscores[j] > scores[j+1]:
                    ...
                    insertCost[j+1] = self.extendgapscore
                    backStrIdx[j+1] = j
                    inserted[j+1] = True
        ...
        for i,node in enumerate(ni()):
            ...
            deleteCost [i+1,1:] = numpy.where(deleted, self.extendgapscore, deleteCost[i+1,1:])
            ...
            insertCost[i+1,inserted] = self.extendgapscore
            deleteCost[i+1,inserted] = self.opengapscore
    ...
    def initializeDynamicProgrammingData(self):
        ...
        # initialize score
        if self.globalAlign:
            scores[0,1:] = self.opengapscore + numpy.arange(0,l2)*self.extendgapscore
            ni = self.graph.nodeiterator()
            for (index, node) in enumerate(ni()):
                prevIdxs = self.prevIndices(node, nodeIDtoIndex)
                scores[0,0] = self.opengapscore - self.extendgapscore
